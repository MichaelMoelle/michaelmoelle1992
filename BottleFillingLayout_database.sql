DROP TRIGGER lager_flaschen_abzug;
DROP TRIGGER bestellungen_abzug;
DROP TRIGGER lager_flaschen_abzug2;
...
FOR EACH ROW
BEGIN
INSERT INTO `Vorratslager`(`Leere Flaschen`,`Flaschendeckel`)
SELECT MIN(`Leere Flaschen`)-1, MIN(`Flaschendeckel`)-1
FROM `Vorratslager`;
END |
DELIMITER ;
...
DO
DELETE FROM `Limonaden_produziert` WHERE `FlaschenID` IN (
SELECT MIN(`FlaschenID`));
